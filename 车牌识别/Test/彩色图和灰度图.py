...
def predict(imageArr):
    ...
    输出：gray_img_原始图像经过高斯平滑后的二值图
          contours是找到的多个轮廓
    """
    img_copy = imageArr.copy()#对原始图片进行复制
    img_copy = hsv_color_find(img_copy)#进行hsv转换。获取到蓝底白字的车牌照，其余的都是黑色的。
    cv2.imshow('img_hsv',img_copy)
    # RGB->灰度
    gray_img = cv2.cvtColor(img_copy, cv2.COLOR_BGR2GRAY)#转换为灰度图，来减少计算大小。
    #cv2.imshow('gray_img', gray_img)
    # 该函数将源图像转换为指定的高斯核。支持就地过滤。
    gray_img_ = cv2.GaussianBlur(gray_img, (5, 5), 0, 0, cv2.BORDER_DEFAULT)
    #cv2.imshow('GaussianBlur', gray_img_)
    kernel = np.ones((23, 23), np.uint8)
    # 使用侵蚀和膨胀作为基本操作来执行高级形态转换。任何操作都可以就地完成.在多通道图像的情况下，每个通道都是独立处理的.
    img_opening = cv2.morphologyEx(gray_img, cv2.MORPH_OPEN, kernel)#进行先腐蚀后膨胀操作，起的作用是：去除黑色中白色的点
    #cv2.imshow("img_opening_open", img_opening)
    # 计算两个数组的加权和
    ...
    ret2, img_thresh2 = cv2.threshold(img_opening, 0, 255, cv2.THRESH_BINARY)#可以理解为简单滤波
    #cv2.imshow('img_thresh',img_thresh)
    #cv2.imshow('img_thresh2', img_thresh2)

    # 该函数在输入图像中查找边缘，并使用Canny算法在输出映射边缘进行标记。阈值1和阈值2之间的最小值用于边缘连接。最大值用于寻找强边的初始段。
    img_edge = cv2.Canny(img_thresh, 100, 200)#边缘检测Canny
    cv2.imshow("img_edge", img_edge)

    # # 使用开运算和闭运算让图像边缘成为一个整体
    # kernel = np.ones((10, 10), np.uint8)
    # 30*30 矩形 其大小需要根据 车牌在图片中宽度的占比和图片像素进行转换， 简测下来大概是 ( 宽占比 * 原图宽像素 / 10 ) 例 0.6 * 500 / 10 = 30
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (10, 10))#定义一个结构，10*10的矩阵。rectangle生成一个矩阵。
    img_edge1 = cv2.morphologyEx(img_edge, cv2.MORPH_CLOSE, kernel)#进行闭运算，sd。首先进行膨胀后进行腐蚀操作，去除白色中的黑色点。
    img_edge2 = cv2.morphologyEx(img_edge1, cv2.MORPH_OPEN, kernel)#进行开元算，ds。首先进行腐蚀后进行膨胀操作，去除黑色中的白色点。
    #这里的操作对象是不相同的
    img_edge3 = cv2.morphologyEx(img_thresh2, cv2.MORPH_CLOSE, kernel)#进行闭运算，sd。首先进行膨胀后进行腐蚀，去除白色中的黑色点。
    img_edge4 = cv2.morphologyEx(img_edge3, cv2.MORPH_CLOSE, kernel)#进行闭运算，sd。首先进行膨胀后进行腐蚀，去除白色中的黑色点。
    ...
    contours2, hierarchy2 = cv2.findContours(img_edge4, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)#轮廓检索模式，定义轮廓层次结构。轮廓近似方法，用于压缩轮廓。
    # print("hierarchy:")
    # print(hierarchy)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
...
def chose_licence_plate(contours, Min_Area=2000):
    ...
    拓展：
    （1） OpenCV自带的cv2.contourArea()函数可以实现计算点集（轮廓）所围区域的面积，用于计算给定轮廓的面积。轮廓是由一系列连续的点组成的闭合曲线。函数声明如下：
            contourArea(contour[, oriented]) -> retval
        其中参数解释如下：
            contour代表输入点集，此点集形式是一个n*2的二维ndarray或者n*1*2的三维ndarray，要计算面积的轮廓，可以是一个包含点的 Numpy 数组。
        返回值：
            retval 表示点集（轮廓）所围区域的面积，轮廓的面积，以像素为单位。
    ...
    for temp_contour in temp_contours:
        rect_tupple = cv2.minAreaRect(temp_contour)#计算给定点集的最小外接矩形，其返回的
    ...
    if len(car_plate1) > 1:
        for temp_contour in car_plate1:
            ...
            # 中国：蓝牌和黑牌是440×140，黄牌前牌尺寸同，后牌为440×220；摩托车及轻便摩托车前牌是220×95，后牌是220×140。
            # 车牌正常情况下宽高比在2 - 3.15之间 稍微放宽点范围
            if aspect_ratio > 1.6 and aspect_ratio < 4.15:#第二次筛选，进行了比例减小。
...
def remove_plate_upanddown_border(card_img):
    ...
    输出: 在高度上缩小后的字符二值图片
    """
    plate_Arr = cv2.imread(card_img)#读取的是 车牌的照片
    plate_gray_Arr = cv2.cvtColor(plate_Arr, cv2.COLOR_BGR2GRAY)#转换格式
    ret, plate_binary_img = cv2.threshold(plate_gray_Arr, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)#使用 cv2.THRESH_BINARY + cv2.THRESH_OTSU 作为阈值类型，表示使用 Otsu's 阈值化方法自动计算阈值，并进行二值化处理。
...
if __name__ == "__main__":
    out_path = '/Users/zhangguowen/Downloads/outpath'
    path = '/Users/zhangguowen/Library/Mobile Documents/com~apple~CloudDocs/pythonProject2/Opencv_Learning/车牌识别/Python_VLPR-master/test/lLD9016.jpg'
    # 你要识别的图片
    ...
    if len(car_plate) == 0:
        ...
        car_img_path = license_segment(car_plate, out_path)
